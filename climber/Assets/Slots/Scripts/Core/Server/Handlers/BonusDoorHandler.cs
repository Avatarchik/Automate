...
namespace Core.Server.Handlers {
    public class BonusDoorHandler : BaseJsonHandler<BonusDoorRequest, BonusDoorResponse> {
        const string FUN_METHOD_URL = "/mobile/game/{0}/{1}";
        const string REAL_METHOD_URL = "/game.json";

        public BonusDoorHandler (BonusDoorRequest request) : base(request) {
        }
        ...
        protected override BonusDoorResponse Deserialize (JObject o) {
            return new BonusDoorResponse (o);
        }

        private string GetGameType() {
            string game;
            if (Request.Game == "garage") {
